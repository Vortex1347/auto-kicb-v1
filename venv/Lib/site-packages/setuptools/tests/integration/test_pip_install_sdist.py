...
"""Integration tests.py for setuptools that focus on building packages via pip.

The idea behind these tests.py is not to exhaustively check all the possible
...
their build process may require changes in the tests.py).
"""
...
EXAMPLES = [
    ...
    # attribute, or modify the tests.py below
]
...
def _prepare(tmp_path, venv_python, monkeypatch, request):
    ...
    def _debug_info():
        ...
        # it is necessary to debug the tests.py directly from the CI logs.
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
