...
class VirtualEnv(jaraco.envs.VirtualEnv):
    ...
    def run(self, cmd, *args, **kwargs):
        ...
        # - tox isn't used to run tests.py and
        ...
        # In that case tests.py look for module in the wrong place (on PYTHONPATH).
...
def run_setup_py(cmd, pypath=None, path=None, data_stream=0, env=None):
    """
    Execution command for tests.py, separate from those used by the
