...
def test_excluded_subpackages(tmpdir_cwd):
    ...
    msg = r"Python recognizes 'mypkg\.tests.py' as an importable package"
    ...
    # Setuptools is configured to ignore `mypkg.tests.py`, therefore the following
    # files/dirs should not be included in the distribution.
    for f in [
        "mypkg/tests.py/__init__.py",
        "mypkg/tests.py/test_mypkg.py",
        "mypkg/tests.py/test_file.txt",
        "mypkg/tests.py",
    ]:
...
def test_existing_egg_info(tmpdir_cwd, monkeypatch):
    ...
    # We need to ensure it is not cached yet, otherwise it may impact on the tests.py
